...
export function svelteWebComponentWrapper(): Plugin {
  return {
    ...
    async transform(code, id) {
      ...
      const checkImport = (
        scriptAst: SvelteAst["instance"] | SvelteAst["module"] | null,
      ) => {
        ...
        walk(scriptAst.content as any, {
          enter(node: any) {
            if (
              ...
              node.source.value === WRAPPER_SOURCE_PACKAGE
            ) {
              if (
                node.specifiers.some(
                  (spec: any) =>
                    spec.type === "ImportSpecifier" &&
                    spec.imported.name === WRAPPER_COMPONENT_NAME,
                )
              ) {
                importExists = true;
                this.skip();
        ...
        if (node.type === "RegularElement" && node.name.includes("-")) {
          ...
          if (!isSelfClosing && !isVoidElement) {
            ...
            if (closingTagStart !== -1 && closingTagStart > node.start) {
              ...
              if (
                openingTagEndMarker !== -1 &&
                closingTagStart > openingTagEndMarker
              ) {
...
export default svelteWebComponentWrapper;
