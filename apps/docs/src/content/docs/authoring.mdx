...
Write a Svelte component and declare its custom-element tag. Svelte defines the
component behavior; svebcomponents reads the source to generate attribute
options, element types, and a custom elements manifest.

Read the
[Svelte custom elements guide](https://svelte.dev/docs/svelte/custom-elements)
for the compiler rules behind these components.

## Declare the tag

Give each published component a literal tag name:
...
name a computed tag during the build.
...
Svelte exposes each prop as a property on the custom element.
...
svebcomponents records static slot names. Read
[Hydration](/server-rendering/hydration/#slots) before you server-render a
component with slots.

## Styles

Svelte puts component styles in the shadow root. Use `:host` for the custom
