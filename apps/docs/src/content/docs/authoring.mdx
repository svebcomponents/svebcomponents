...
Scalar (primitive type) props reflect to attributes. `auto-options` disables reflection for
...
Svelte's custom element compiler puts component styles in the shadow root. As with all custom elements, use `:host` for the custom
