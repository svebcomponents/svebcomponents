...
:::caution[Beta]
SSR is ready for real-world evaluation and early production adoption, but its
APIs and generated output may still change before 1.0. Attribute values, names
and tag names are validated and escaped through Svelte's SSR serializer and
covered by XSS regression tests, but the package has not had an independent
security audit. Shadow-root content rendered via `{@html}` relies on the
renderer's escaping, so applications handling untrusted content should assess
...
[static tags only](/core-concepts/build/#static-tags-only)), the generated
...
server-render it. There are exactly two ways onto the async path:
...
2. **Its preparation hook returns a promise** — the adjacent
   ...
   [server preparation](/core-concepts/build/#server-preparation) module,
...
would have fetched, keeps the synchronous path available, which is
...
The examples above use Svelte. Nothing about the element is Svelte-specific;
only the host application's wiring is. For Vue, and for building an
integration against another framework, see
[Framework Integrations](/core-concepts/ssr/framework-integrations/).
