...
That's it — no explicit registration call needed. The generated renderer
knows its own tag name (read from the component's `<svelte:options
customElement>` declaration at build time) and registers itself with
`ElementRendererRegistry` as soon as it's imported. The DOM shim is
guaranteed to install first regardless of import order or how a bundler
chunks things, so there's no ordering convention to get right here.
